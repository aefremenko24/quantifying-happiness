...
struct SatisfactionEntryView: View {
    ...
    var body: some View {
        VStack(alignment: .center) {
            if (date > Date()) {
                Text("This date is in the future, come back later")
            }
            else if (metrics == nil) {
    ...
    private func updateSatisfactionEntry() {
        if metrics != nil && satisfactionEntry != nil {
            let newEntry = SatisfactionEntry(from: metrics!, date: date, satisfactionScore: Double(currentSatisfactionScore!))
